# -*- coding: utf-8 -*-
"""OpenClaw와 Hermes Agent 해부 전용 PDF 빌더 — Books/ 경로 대응 + 마크다운 표 렌더링.

build_pdf_security.py와 동일한 렌더러(표지 자동 축소, 파이프 표, 코드 블록, footer).
표지·목차·들어가며·Phase/Section 개요·Topic 본문을 렌더링한다.
"""
import json
import re
...
BOOK_DIR = Path("C:/Users/cyon1/OneDrive/Desktop/Autobook/Books/OpenClaw와 Hermes Agent 해부")
PAGES_DIR = BOOK_DIR / "wikidocs" / "pages"
CUR = json.load(open(BOOK_DIR / "curriculum.json", encoding="utf-8"))

TITLE = "OpenClaw와 Hermes Agent 해부"
SUBTITLE = "상주형 개인 AI 에이전트는 어떻게 설계되는가"
KEYWORDS = "Gateway · Heartbeat · Cron · Skill · Memory · 심층 방어"
YEAR = "2026"
SERIES = "A U T O B O O K   S E R I E S"
OUT = BOOK_DIR / "OpenClaw와 Hermes Agent 해부.pdf"
...
for phase in CUR["phases"]:
    pdf.set_x(18)
    pdf.set_font("M", "B", 11.5)
    pdf.multi_cell(0, 7, f"{phase['id']} {phase['title']}", new_x="LMARGIN", new_y="NEXT")
    for section in phase["sections"]:
        pdf.set_x(23)
        pdf.set_font("M", "B", 10.0)
        pdf.multi_cell(0, 5.8, f"{section['id']} {section['title']}", new_x="LMARGIN", new_y="NEXT")
        for topic in section["topics"]:
            pdf.set_x(28)
            pdf.set_font("M", "", 9.4)
            pdf.multi_cell(0, 5.2, f"{topic['id']} {topic['title']}", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(1.5)
...
for phase in CUR["phases"]:
    ...
    for section in phase["sections"]:
        pdf.set_x(24)
        pdf.set_font("M", "", 10.3)
        pdf.multi_cell(0, 5.6, f"{section['id']} {section['title']}", new_x="LMARGIN", new_y="NEXT")
        for topic in section["topics"]:
            pdf.set_x(30)
            pdf.set_font("M", "", 9.6)
            pdf.multi_cell(0, 5.2, f"{topic['id']} {topic['title']}", new_x="LMARGIN", new_y="NEXT")
...
if missing:
    ...
    print("[INFO] all 72 topics rendered")
