...
1. `교재/$ARGUMENTS/curriculum.json`이 이미 존재하는지 확인한다.
   - 존재하면: 로드 후 **3단계(검증/개선)**부터 재개한다.
   - 없으면: 2단계부터 시작한다.
2. `교재/$ARGUMENTS/` 디렉토리가 없으면 생성한다.
3. `교재/$ARGUMENTS/._research/` 디렉토리가 없으면 생성한다.
4. `교재/$ARGUMENTS/wikidocs/` 디렉토리가 이미 존재하는지 확인한다 (resume 대응).
...
책이름: {$ARGUMENTS}
출력 경로: 교재/{$ARGUMENTS}/._research/curriculum_research.md
...
리서치 완료 후 `교재/{$ARGUMENTS}/._research/curriculum_research.md`를 읽어 조사 결과를 파악한다.
...
`교재/$ARGUMENTS/curriculum.json`에 저장한다.
