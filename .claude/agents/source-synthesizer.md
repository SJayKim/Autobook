...
1. `{출력 디렉토리}/_meta/keyword_map.json`에서 클러스터 목록을 읽는다.
2. 각 클러스터에 해당하는 소스만 필터링하여 읽는다.
3. 클러스터별 중간 요약을 `{출력 디렉토리}/synthesis/_cluster_{id}.md`에 저장한다.
...
Phase A를 수행한 경우:
- `synthesis/_cluster_*.md` 중간 요약들을 읽어 종합한다.

Phase A를 건너뛴 경우:
- `{출력 디렉토리}/sources/` 하위의 모든 .md 파일을 직접 읽는다.
