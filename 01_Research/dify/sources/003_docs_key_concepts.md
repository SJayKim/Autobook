---
source_id: "003"
title: "Dify Official Documentation - Key Concepts"
url: "https://docs.dify.ai/en/use-dify/getting-started/key-concepts"
type: "documentation"
scraped_at: "2026-03-27"
keywords: ["Dify platform overview", "Dify application types"]
content_length: 5890
---
...
User Input and Trigger Start nodes are mutually exclusive -- they cannot be used on the same canvas. To switch between them, right-click the current start node and select Change Node. Alternatively, delete the current start node and add a new one.
...
### Inputs
You can specify any number of input variables at the User Input node for your app's end users to fill in. User inputs are set at the start of each workflow run and cannot be updated.

### System Variables (Workflow)
- `sys.user_id` - User ID: unique identifier for each user
- `sys.app_id` - App ID: unique identifier for each App
- `sys.workflow_id` - Workflow ID: records all nodes information
- `sys.workflow_run_id` - Workflow Run ID: records runtime status and execution logs
- `sys.timestamp` - Start time of each workflow execution

### System Variables (Chatflow)
- `sys.conversation_id` - Unique ID for the chatting box interaction session
- `sys.dialogue_count` - Number of conversation turns during interaction
- `sys.user_id` - Unique ID for each application user
- `sys.app_id` - App ID
- `sys.workflow_id` - Workflow ID
- `sys.workflow_run_id` - Workflow Run ID

### Outputs
Each node produces one or more outputs that can be referenced in subsequent nodes.

### Environment Variables
Store sensitive information like API keys specific to your app. This allows a clean separation between secrets and the Dify app itself, so you don't have to risk exposing passwords and keys when sharing your app's DSL. Environment variables are also constants and cannot be updated.

### Conversation Variables (Chatflow only)
These variables are conversation-specific, meaning they persist over multi-turn chatflow runs in a single conversation so you can store and access dynamic information like to-do list and token cost. You can update the value of a conversation variable via the Variable Assigner node.
