---
source_id: "033"
title: "Dify v1.6.0: Built-in Two-Way MCP Support"
...
Published: Jul 10, 2025 | Author: Leilei, Product Marketing
...
The Model Context Protocol (MCP) standardizes how AI agents discover and use outside servers. Earlier you had to invoke MCP tools through a plugin. In the latest Dify release (v1.6.0), MCP is built in both directions:
...
Define the agent's role in the prompt and attach the MCP server. For example:
...
Insert an agent node into a workflow. The agent chooses the right MCP tools at run-time. Ideal for complex, varied tasks such as triaging user feedback into three specialized agents:
...
When a feedback item reaches its dedicated agent, the agent instantly sorts the content, sets its priority, and uses Linear's MCP APIs to open the right issue in the correct team project.
